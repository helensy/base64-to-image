var base64ToImage = require('../index');
var should = require('should');
var fs = require('fs');

describe('Base64 to image', function () {
	it('should throw an error when base64 argument is null', function (done) {
		base64ToImage(null, '/opt', {'fileName':'test'}).catch((e) => {
			should(e.message).be.exactly('Missing mandatory arguments base64 string and/or path string')
			done();
		});
	});
	
	
	it('should throw an error when path argument is null', function (done) {

		base64ToImage(null, null, {'fileName':'test'}).catch((e) => {
			should(e.message).be.exactly('Missing mandatory arguments base64 string and/or path string')
			done();
		});
	});

	it('should throw an error when an invalid base64 argument is passed', function (done) {
		base64ToImage('invalid string', '/opt', {'fileName':'test'}).catch((e) => {
			should(e.message).be.exactly('Invalid base64 string');
			done();
		});
	}); 
	
	it('should decode base64 and save image to disk', function (done) {
		this.timeout(10000);
		var base64Str = 'data:image/png;base64,';
		var path =  __dirname + '/';
		var fileSaved = false;
		base64ToImage(base64Str, path, {'fileName':'image_test'}).then((file) => {
			file.should.have.property('imageType', 'png');
			file.should.have.property('fileName', 'image_test.png');
			return new Promise((resolve, reject) => {
				fs.access('./test/image_test.png',  fs.constants.R_OK | fs.constants.W_OK, (err) => {
					if(err) {
						reject(err);
						return;
					}
					resolve();
				});
			});
		})
		.then(() => {
			done();
		});
	});
});